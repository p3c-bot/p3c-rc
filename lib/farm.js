...
alertify.defaults.notifier.delay = 60
        ...
        farmContract.myCrop.call(function (err, result) {
            ...
            if (onboard && (err || (result == '0x0000000000000000000000000000000000000000'))) {
                alertify.confirm(
                    ...
                    `
                    <p class="agreement">
                    P3C is an autonomous smart contract. Read it <a target="_blank" href="http://etherhub.io/addr/0xae5433263a626f397fed88421cc85ffd22bbc8dd#tab_addr_3">here</a>. By agreeing, you accept that the contract will be run exactly as written, and that <b>code is law</b>.
                    To accept you need an <a target="_blank" href="https://chrome.google.com/webstore/detail/saturn-wallet/nkddgncdjgjfcddamfgcmfnlhccnimig">Ethereum Classic Wallet</a> with ETC in it.
                    </p>
                    <img src="img/etc-logo.png" class="ui image etc-logo" />
                    `,
                    //if ok deploy the crop
                    function () {
                        alertify.success("Thank you. Please accept the transaction to get started.")
...
function getMyCropTokens() {
    farmContract.myCropTokens.call(function (err, result) {
        if (!err) {
            ...
            myCropTokens = result;
            if (change) {
                $("#myCropTokens").replaceWith("<b id='myCropTokens'>" + (web3.fromWei(myCropTokens) * .995).toFixed(2) + "</b>")
    ...
    farmContract.createCrop.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                ...
                alertify.error("Agreement declined.")
                console.log(error);
            }
        })
}
